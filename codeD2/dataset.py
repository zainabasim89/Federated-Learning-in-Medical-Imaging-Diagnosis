...
# Native multiclass task with a pre-made train/test split on
# disk (exact folder names auto-detected, so slight naming
# differences in your download don't require code changes).
#
...
#
# NOTE: if your dataset does NOT come with a pre-made
# train/test split on disk (flat "one folder per class" layout
# instead), use FlatFolderDataset + global_train_test_split
# below instead of this class.
#
...
# =========================================================
# FLAT FOLDER DATASET  (e.g. Kaggle "Otoscopic_Data")
#
# For datasets that do NOT ship a pre-made train/test split —
# just one folder per class, all images together:
#
#   root_dir/
#     <ClassFolder1>/   e.g. "Normal"
#     <ClassFolder2>/   e.g. "Earwax"
#     <ClassFolder3>/
#     <ClassFolder4>/
#     <ClassFolder5>/
#
# Any number of class folders is supported (auto-detected,
# sorted alphabetically for reproducibility) — works for the
# 5-class Otoscopic_Data set (4 disease classes + Normal) just
# as it would for 4 or 6.
#
# Same public interface as EarDiseaseDataset (.samples,
# .class_to_idx, .idx_to_class, .num_classes, .class_names()),
# so it's a drop-in for everything downstream (TransformSubset,
# dirichlet_partition, iid_partition, save_partition_summary).
#
# Since there's no train/test split on disk, pair this with
# global_train_test_split() below to carve out a held-out test
# set BEFORE any FL partitioning happens.
# =========================================================
class FlatFolderDataset(Dataset):
    def __init__(self, root_dir, class_to_idx=None):
        ...
        if not os.path.exists(root_dir):
            raise FileNotFoundError(
                f"[FlatFolderDataset] Folder not found: {root_dir}\n"
                f"Expected structure:\n"
                f"  {root_dir}/<ClassFolder1>/\n"
                f"  {root_dir}/<ClassFolder2>/\n"
                f"  ...\n"
                f"Update ROOT in run_flower.py if your folder is named "
                f"differently."
            )
        ...
        if not class_folders:
            raise RuntimeError(
                f"[FlatFolderDataset] No class subfolders found in "
                f"{root_dir}"
            )
    ...
    def _print_summary(self):
        ...
        print(f"\n[Dataset:FlatFolder] Total: {total} | "
              f"Classes: {self.num_classes}")
        for cname in sorted(by_class.keys(),
                             key=lambda c: self.class_to_idx[c]):
            idx = self.class_to_idx[cname]
...
# SAMPLE LIST VIEW
#
# Lightweight wrapper around a pre-filtered list of
# (path, class_idx, class_name) samples. Exposes the exact
# same interface as EarDiseaseDataset / FlatFolderDataset
# (.samples, .class_to_idx, .idx_to_class, .num_classes,
# .class_names()), so it's a drop-in for `full_dataset` in
# dirichlet_partition / iid_partition / save_partition_summary
# and for wrapping in TransformSubset.
#
# Used to carve the train-pool and held-out-test views out of
# a single FlatFolderDataset (see global_train_test_split).
# =========================================================
class SampleListView(Dataset):
    def __init__(self, samples, class_to_idx, name="Subset"):
...
# =========================================================
# GLOBAL TRAIN-POOL / TEST SPLIT  (stratified)
#
# For datasets with NO pre-made train/test split (flat
# folder-per-class layout). Carves out a held-out global test
# set FIRST, stratified by class, seeded for reproducibility.
# The remaining "train pool" is what later gets partitioned
# across FL clients (via dirichlet_partition / iid_partition),
# which themselves further split each client's share into
# train/val using `val_ratio`.
#
# test_ratio and val_ratio are both expressed as a fraction of
# the FULL dataset (not of the remaining pool), so the three
# numbers you pass in are the literal overall percentages you
# want, e.g. test_ratio=0.15, val_ratio=0.15 -> 70/15/15 train/
# val/test. (val_ratio is converted internally into a fraction
# of the train pool for use with dirichlet_partition/
# iid_partition's own val_ratio argument — see
# `pool_val_ratio()` below.)
# =========================================================
def global_train_test_split(full_dataset, test_ratio=0.15, seed=42):
    """
    Stratified split of a flat dataset into:
      - train_pool_view : SampleListView, used as `full_dataset`
                          input to dirichlet_partition/iid_partition
                          for per-client FL partitioning
      - test_view       : SampleListView, the locked global
                          held-out test set (never touched by
                          any client, evaluated once per round
                          on the server)

    test_ratio is the fraction of the WHOLE dataset held out as
    the global test set.
    """
    set_seed(seed)

    labels  = np.array([s[1] for s in full_dataset.samples])
    indices = np.arange(len(full_dataset.samples))

    splitter = StratifiedShuffleSplit(
        n_splits=1, test_size=test_ratio, random_state=seed
    )
    train_pool_pos, test_pos = next(splitter.split(indices, labels))

    train_samples = [full_dataset.samples[i] for i in train_pool_pos]
    test_samples  = [full_dataset.samples[i] for i in test_pos]

    train_pool_view = SampleListView(
        train_samples, full_dataset.class_to_idx,
        name=f"Train Pool ({(1-test_ratio)*100:.0f}%)"
    )
    test_view = SampleListView(
        test_samples, full_dataset.class_to_idx,
        name=f"Global Test ({test_ratio*100:.0f}%)"
    )

    print(f"\n[GlobalSplit] Stratified | test_ratio={test_ratio} | "
          f"pool={len(train_samples)} | test={len(test_samples)}")

    return train_pool_view, test_view


def pool_val_ratio(test_ratio, target_val_ratio_of_total):
    """
    Converts a desired val fraction OF THE WHOLE DATASET into
    the equivalent val fraction of the remaining train pool
    (i.e. after the global test split has already been
    removed), for use as the `val_ratio` argument to
    dirichlet_partition / iid_partition.

    e.g. test_ratio=0.15, target_val_ratio_of_total=0.15
         -> pool is 85% of total
         -> returns 0.15 / 0.85 = 0.1765
         -> train/val/test overall = 70% / 15% / 15%
    """
    pool_fraction = 1.0 - test_ratio
    if pool_fraction <= 0:
        raise ValueError("test_ratio must be < 1.0")
    return target_val_ratio_of_total / pool_fraction
